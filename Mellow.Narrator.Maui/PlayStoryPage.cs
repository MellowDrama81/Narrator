...
public sealed class PlayStoryPage : ContentPage, IPlayStoryTabStatePage, IPendingOperationPage, ICloseGuardPage, IInFlightRequestPage
{
        ...
        _busyOverlay = new Grid
        {
            ...
            Children =
            {
                new HorizontalStackLayout
                {
                    ...
                    Children = { _busy, new Label { Text = "Writingâ€¦", TextColor = Colors.White } }
                }
            }
        };
    ...
    private async Task RefreshCoreAsync(bool scrollToLatestTurn)
    {
        ...
        try
        {
            ...
            if (!StoryBibleProcessor.IsWithinLimits(state.CurrentStoryBible, settings.StoryGeneration))
            {
                ...
                var names = string.Join(Environment.NewLine, preview.Changes.Select(x => $"â€¢ {x.Before?.Name}"));
            ...
            if (!PlannedEventProcessor.IsWithinLimits(state.CurrentPlannedEvents, settings.StoryGeneration))
            {
                ...
                var descriptions = string.Join(Environment.NewLine, preview.Changes.Select(x => $"â€¢ {x.Before?.Description}"));
    ...
    private async Task ShowMetConditionsAsync(StoryState state, StoryTurn turn)
    {
        ...
        if (metVictories.Count > 0) lines.Add("Victory condition(s) met:" + Environment.NewLine + string.Join(Environment.NewLine, metVictories.Select(x => $"â€¢ {x}")));
        if (metLosses.Count > 0) lines.Add("Loss condition(s) met:" + Environment.NewLine + string.Join(Environment.NewLine, metLosses.Select(x => $"â€¢ {x}")));
