...
internal static class ImportExportService
{
    ...
    {
        var lines = new List<string> { $"{group.Header} â€” {group.At.ToLocalTime():g}" };
    ...
    private static string FormatPlannedEventChange(AppliedPlannedEventChange change) =>
        ...
        $"Before: {change.Before?.Description ?? "â€”"}" + Environment.NewLine +
        $"After: {change.After?.Description ?? "â€”"}";
